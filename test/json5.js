...
describe('bodyParser.json5()', function(){
  it('should parse JSON5', function(done){
    var server = createServer({ limit: '1mb' })

    request(server)
    .post('/')
    .set('Content-Type', 'application/json')
    .send('{user:"tobi"}') // JSON5 behavior
  ...
  it('should fail gracefully', function(done){
    ...
    .set('Content-Type', 'application/json')
    .send('{"user"')
    .expect(400, 'Expected \':\' instead of \'\'', done)
  })
  ...
  it('should 400 on malformed JSON', function(done){
    ...
    .set('Content-Type', 'application/json')
    .send('{:')
    .expect(400, 'Bad identifier', done);
  })
