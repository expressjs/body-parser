...
function extendedparser(options) {
  var parameterLimit = sanitizeNumberOption('parameterLimit', 1000, options)
  var arrayLimit = sanitizeNumberOption('arrayLimit', 100, options)
  var parse = parser('qs')
  var opts = {
...
function sanitizeNumberOption(name, def, options) {
  var arg = options[name] !== undefined
    ? options[name]
    : def
  var parse = parser('querystring')

  if (isNaN(arg) || arg < 1) {
    throw new TypeError('option ' + name + ' must be a positive number')
  }
...
function simpleparser(options) {
  var parameterLimit = sanitizeNumberOption('parameterList', 1000, options)
  var parse = parser('querystring')
