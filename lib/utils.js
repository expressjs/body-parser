 ...
 *
 * Per RFC 7159 section 8.1, JSON text SHALL be encoded in UTF-8, UTF-16, or UTF-32.
 *
...
function isValidJsonCharset (charset) {
  ...
  return normalized === 'utf-8' || normalized === 'utf-16' || normalized === 'utf-32'
}

/**
 * Determines whether a charset is permitted for application/x-www-form-urlencoded payloads.
 *
 * Per HTML Living Standard, the only supported encodings are UTF-8 and ISO-8859-1 (case-insensitive).
 *
 * @param {string} charset
 * @returns {boolean}
 * @private
...
function isValidUrlencodedCharset (charset) {
  if (!charset) return false
  const normalized = charset.toLowerCase()
  return normalized === 'utf-8' || normalized === 'iso-8859-1'
}
